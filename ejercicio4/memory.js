...
class MemoryGame {
  ...
  checkForWin() {
    if (this.matchedCards.length === this.board.cards.length) {
      ...
      alert(
        `¡Felicidades! Has ganado en ${this.moves} movimientos y ${this.timeElapsed} segundos.`
      );
    }
  }
  resetGame() {
  ...
  updateMoveCounter() {
    ...
    moveCounter.textContent = `Movientos: ${this.moves}`;
  }
  updateTimerDisplay() {
    const timerDisplay = document.getElementById("timer-display");
    timerDisplay.textContent = `Tiempo: ${this.timeElapsed} segundos`;
  }
}
